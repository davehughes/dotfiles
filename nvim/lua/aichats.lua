...
function telescope_picker(opts)
  opts = opts or {}
...
function path_display_with_right_aligned_text(primary_text, right_aligned_text)
  local status = get_status(vim.api.nvim_get_current_buf())
  local available_width = vim.api.nvim_win_get_width(status.layout.results.winid) - status.picker.selection_caret:len() - 2
  local min_gap = 2
...
function short_label_from_path(path)
...
function filepath_for_current_buffer()
...
function generate_filepath_for_chat_buffer(file_prefix)
  return string.format("%s/%s.%d.aichat", aichats_default_directory, file_prefix, os.date("%Y%m%d"))
end

function save_current_chat_buffer()
