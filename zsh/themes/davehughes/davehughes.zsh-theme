...
function combined_location {
    ...
    echo "in combined location" >> /tmp/zlog
    ...
    echo "looking at lpath: [${LPATH}]" >> /tmp/zlog
    if [ "${#LPATH}" != "0" ]; then
	# TODO: this is broken for some reason, fix it to get color back
        # SEGMENTS+=("%{$C[path]%}${LPATH}")
        SEGMENTS+=("${LPATH}")
    fi

    ## Join segments with separators - man, am I terrible at zsh scripting!
    echo "looking at final join: [${SEGMENTS}]" >> /tmp/zlog
